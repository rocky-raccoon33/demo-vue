<template>
    <card title="数据集管理">
        ...
        <el-dialog title="上传数据集" :visible.sync="uploadDialog" width="30%" :append-to-body="true" class="custom-dialog"
            ...
            <el-upload class="w-100" drag action="http://1.14.204.181:5000/receive_dataset_data" :limit="1" v-else
    ...
    methods: {
        ...
        delete_dataset(index, row) {
            this.$confirm('确定删除？')
                .then(() => {
                    //后端操作
                    fetch(`http://1.14.204.181:5000/delete_dataset`, {
                        method: 'post',
                        body: JSON.stringify({
        ...
        update_dataset(row, key) {
            fetch(`http://1.14.204.181:5000/update_dataset`, {
                method: 'post',
                body: JSON.stringify({
                    'dataset_id': row.dataset_id,
                    'key': key,
        ...
        update_location(row, location, longitude, latitude) {
            fetch(`http://1.14.204.181:5000/update_location`, {
                method: 'post',
                body: JSON.stringify({
                    'dataset_id': row.dataset_id,
                    'location': location,
        ...
        submitForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    fetch(`http://1.14.204.181:5000/add_dataset`, {
                        method: 'post',
                        body: JSON.stringify({
