...
export default {
  ...
  methods: {
    ...
    getCorrlationData(Number, y_name, x_name) {
      if(!Number) return
      fetch(`http://http://1.14.204.181:5000/correlation?number=${Number}&y=${y_name}&x=${x_name}`,{
        headers:{
    ...
    fetchStatsCardsData(windTurbineName) {
      ...
      fetch('http://http://1.14.204.181:5000/basic_info?number=' + windTurbineName,{
        headers:{
          'Content-Type': 'application/json', // 设置内容类型头部信息为 JSON
          'Authorization': `Bearer ${this.$cookies.get('token')}`, // 设置授权头部信息
        }
    ...
    toggleActive(value) {
      ...
      const fetchXAxisData = fetch(`http://http://1.14.204.181:5000/dimension_data?number=${windTurbineName}&dimension=DATATIME`,{
        headers:{
          'Content-Type': 'application/json', // 设置内容类型头部信息为 JSON
          'Authorization': `Bearer ${this.$cookies.get('token')}`, // 设置授权头部信息
        }
      ...
      const fetchYAxisData = fetch(`http://http://1.14.204.181:5000/dimension_data?number=${windTurbineName}&dimension=${value}`,{
        headers:{
          'Content-Type': 'application/json', // 设置内容类型头部信息为 JSON
          'Authorization': `Bearer ${this.$cookies.get('token')}`, // 设置授权头部信息
        }
