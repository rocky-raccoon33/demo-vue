<template>
    <div class="row">
        <div class="col-12 col-lg-6">
            <card title="导入数据">
                <el-upload class="w-100" drag action="http://1.14.204.181:5000/receive_predict_data" :limit="1"
...
export default {
    ...
    methods: {
        ...
        getModels() {
            ...
            fetch(`http://1.14.204.181:5000/get_models?dataset=${dataset}`, {
                headers: {
        ...
        predict() {
            ...
            fetch(`http://1.14.204.181:5000/predict?analyst=${analyst}&number=${number}&score=${score}&model_type=${model_type}&model_id=${model_id}`, {
                headers: {
                    'Content-Type': 'application/json', // 设置内容类型头部信息为 JSON
                    'Authorization': `Bearer ${this.$cookies.get('token')}`, // 设置授权头部信息
                }
        ...
        //下载
        downloadResult() {
            fetch(`http://1.14.204.181:5000/get_predict_csv`, {
                headers: {
                    'Content-Type': 'application/json', // 设置内容类型头部信息为 JSON
                    'Authorization': `Bearer ${this.$cookies.get('token')}`, // 设置授权头部信息
                },
        ...
        'preOption': {
            handler() {
                ...
                if (preChart && preChart.setOption) {
                    preChart.setOption(this.preOption); //更新日期销量曲线图
                }
            },
            deep: true
        },
