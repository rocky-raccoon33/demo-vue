...
export default {
  ...
  methods: {
    ...
    getUnprocessedData(Number) {
      fetch(`http://http://1.14.204.181/:5000/unprocessed_data?number=${Number}`, {
        headers: {
    ...
    async train() {
      ...
      await fetch(`http://http://1.14.204.181/:5000/train`, {
        method: 'post',
        body: JSON.stringify({
    ...
    getTrainedData(Number) {
      fetch(`http://http://1.14.204.181/:5000/trained_data`, {
        method: 'post',
        body: JSON.stringify({
          number: Number,
          samples: this.samples,
    ...
    retrain() {
      const analyst = 'rich'
      fetch(`http://http://1.14.204.181/:5000/retrain?analyst=${analyst}`, {
        headers: {
          'Content-Type': 'application/json', // 设置内容类型头部信息为 JSON
          'Authorization': `Bearer ${this.$cookies.get('token')}`, // 设置授权头部信息
        }
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          fetch(`http://http://1.14.204.181/:5000/save_model`, {
            method: 'post',
            body: JSON.stringify({
    ...
    'validOption': {
      handler() {
        ...
        if (validChart && validChart.setOption) {
          validChart.setOption(this.validOption); //更新日期销量曲线图
        }
      },
      deep: true
    },
    ...
    'trainOption': {
      handler() {
        ...
        if (trainChart && trainChart.setOption) {
          trainChart.setOption(this.trainOption); //更新日期销量曲线图
        }
      },
      deep: true
    },
