...
export default {
  ...
  methods: {
    ...
    fetchBinProcessedData(number, sigma, deadCount, step, missingValueOption, aValueOption, bValueOption) {
      fetch(`http://http://1.14.204.181/:5000/bin_data?number=${number}&sigma=${sigma}&deadCount=${deadCount}&step=${step}`,{
        headers:{
    ...
    doDataProcess() {
      ...
      fetch(`http://http://1.14.204.181/:5000/do_data_process?missingValueOption=${missingValueOption}&aValueOption=${aValueOption}&bValueOption=${bValueOption}`,{
        headers:{
          'Content-Type': 'application/json', // 设置内容类型头部信息为 JSON
          'Authorization': `Bearer ${this.$cookies.get('token')}`, // 设置授权头部信息
        }
